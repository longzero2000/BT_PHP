...
abstract class Hinh{
	...
	public function setDorong($doRong){
		$this->dorong=$doRong;
	}
	public function getDorong(){
		return $this->dorong;
	}
...
class HinhCN extends Hinh{
	public function tinh_CV(){
		return $this->dodai+dorong*2;
	}
	public function tinh_DT(){
		return $this->dodai*dorong;
	}
}
$str=NULL;
if(isset($_POST['tinh'])){
	...
	if(isset($_POST['hinh']) && ($_POST['hinh'])=="ch"){
		$ch=new HinhCN();
		$ch->setTen($_POST['ten']);
		$ch->setDodai($_POST['dodai']);
		$ch->setDorong($_POST['dorong']);
		$str= "Diện tích hình chữ nhật ".$ch->getTen()." là : ".$ch->tinh_DT()." \n".
		 		"Chu vi của hình chữ nhật ".$ch->getTen()." là : ".$ch->tinh_CV();
	}
	...
	<table border='0'>
		<tr>
			...
			<td><input type="radio" name="hinh" value="hv" 
				...
				<input type="radio" name="hinh" value="ch" 
					<?php if(isset($_POST['hinh'])&&($_POST['hinh'])=="ch") echo 'checked'?>/>Hình chữ nhật
			</td>
		</tr>
		<tr>
			<td>Nhập tên:</td>
		...
		<tr>
			...
			<td><input type="number"  name="dodai" value="<?php if(isset($_POST['dodai'])) echo $_POST['dodai'];?>"/></td>
		</tr>
		<tr>
			<td>Nhập chiều rộng:</td>
			<td><input type="number"  name="dorong" value="<?php if(isset($_POST['dorong'])) echo $_POST['dorong'];?>"/></td>
		</tr>
		<tr><td>Kết quả:</td>
